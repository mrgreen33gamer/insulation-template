  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        if (p.flake) {
          ctx.strokeStyle = '#4f46e5'; ctx.lineWidth = 0.6;
          ctx.translate(p.x, p.y);
          for (let i = 0; i < 6; i++) { ctx.rotate(Math.PI / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, p.r * 3.2); ctx.stroke(); }
        } else { ctx.fillStyle = '#4f46e5'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
        ctx.restore();
        p.x += p.vx; p.y += p.vy;
  ...
  return (
    <div className={styles.thermo} aria-hidden="true">
      ...
      <div className={styles.thermoLabels}>
        <span className={styles.thermoTop}>Cool</span>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Most Trusted Insulation Pros — Since 2012
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Cooler Homes.<br />Lower Bills.<br />
            <span className={styles.accentLine}>ThermoShield Insulation.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            Attic · Wall · Spray Foam Insulation. Flat-rate pricing. Clean installs.
            Comfort &amp; Efficiency Guarantee · 5-Year Workmanship. Serving Waco and Central Texas with BPI-aligned installers.
          </motion.p>

          <motion.div className={styles.ctaRow}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.34 }}>
            <a href="tel:+12548505050" className={styles.ctaPrimary}>
